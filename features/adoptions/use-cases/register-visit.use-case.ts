...
import { registerVisitSchema } from '../schemas/adoption.schema';
import { AdoptionError } from '../domain/errors/adoption.error';

export class RegisterAdoptionVisitUseCase {
  constructor(private readonly adoptionRepository: AdoptionRepository) {}

  async execute(data: unknown): Promise<AdoptionEntity> {
    const parseResult = registerVisitSchema.safeParse(data);
    if (!parseResult.success) {
      throw new AdoptionError('Invalid register visit data', 'INVALID_INPUT', parseResult.error.format());
    }

    const { adoptionId, visitedAt } = parseResult.data;
    ...
    existingAdoption.registerVisit(visitedAt);
